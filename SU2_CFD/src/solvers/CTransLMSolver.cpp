...
//  cout << "CTransLMSolver::Source_Residual" << endl;
    ...
    for (unsigned long iPoint = 0; iPoint < nPointDomain; iPoint++) {
        ...
        if(nDim == 3)
          Velocity[2] = flowNodes->GetVelocity(iPoint, 2);

//        cout << "Qui" << endl;

        nodes->SetQuantities(iPoint, constants, flowNodes->GetLaminarViscosity(iPoint), geometry->nodes->GetWall_Distance(iPoint), flowNodes->GetDensity(iPoint), VorticityMag, flowNodes->GetStrainMag(iPoint), flowNodes->GetVelocityGradient(iPoint), Velocity, turbNodes->GetSolution(iPoint));
