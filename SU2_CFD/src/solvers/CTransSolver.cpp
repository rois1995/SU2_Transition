...
/*--- Explicit instantiation of the parent class of CTransSolver. ---*/
template class CScalarSolver<CTransLMVariable>;

CTransSolver::CTransSolver(CGeometry* geometry, CConfig *config, bool conservative)
  : CScalarSolver<CTransLMVariable>(geometry, config, conservative) {
...
void CTransSolver::LoadRestart(CGeometry** geometry, CSolver*** solver, CConfig* config, int val_iter,
                              bool val_update_geo) {
...
void CTransSolver::Impose_Fixed_Values(const CGeometry *geometry, const CConfig *config){
